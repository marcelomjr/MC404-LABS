...
configure_gpio:
	...
	@ confiugure input/output pins
	ldr r0, =0xFFFC003E  
	ldr r1, =GPIO_BASE

	str r0, [r1, #GPIO_DIR]

        @ interrupt programmed only to bit0
        mov r0, #2
        str r0, [r1, #GPIO_ICR1]
        ldr r0, =0xFFFFFFFF
        str r0, [r1, #GPIO_ISR]
...
@  'r0' output register: read value
@ --------------------------------------------------------------------------------
.globl nonblocking_read_sonar

  stmfd sp!, {r4-r11,lr}

        
@@@@@@@@@@@ RESET TRIGGER AND SET MUX @@@@@@@@@@@@@@@@@
nonblocking_read_sonar: 
